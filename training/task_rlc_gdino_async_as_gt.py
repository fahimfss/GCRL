...
def parse_args():
    ...
    parser.add_argument('--img_aug_path', default='/home/fahim/project/robotx/images_array_120_160.npy', type=str)
...
def main(seed=-1, env_name=None):
    ...
    state = env.reset(create_vid=False, action_scale=action_scale, roi=roi)
    ...
    while env.total_steps < args.env_steps:
        ...
        if env.total_steps > 0 and env.total_steps % 10000 == 0:
            action_scale += 0.04
            action_scale = min(action_scale, 0.6)
        if  roi == 0 and env.total_steps > 100000:
            roi = 1
        ...
        if args.eval_steps > 0 and env.total_steps % args.eval_steps == 0:
            ...
            eval_data = {'params': agent.get_actor_params(), 
                         'step': env.total_steps, 
                         'action_scale': action_scale, 
                         'roi': roi}
            
            eval_queue_2.put(eval_data) 
            time.sleep(10)
            eval_queue_2.get()

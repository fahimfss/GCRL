...
class MujocoEnv(gym.Env, gym.utils.EzPickle, ObsVecDict):
    ...
    def forward(self, image, **kwargs):
        ...
        #print(image.size)
        obs = {'image': image, 'vector': obs.astype(np.float32)}
    ...
    def get_env_infos(self):
        ...
        if hasattr(self, 'classifier') and (self.classifier == 'gdino' or self.classifier == 'detic' or self.classifier == 'owlv2'):            
            env_info = {
                ...
                'cf_step':self.gs,
                'cf_time':self.cf_time,
                'cf_accuracy':self.cf_accuracy,

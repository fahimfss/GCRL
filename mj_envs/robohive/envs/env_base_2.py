...
class MujocoEnv(gym.Env, gym.utils.EzPickle, ObsVecDict):
    ...
    def _setup(self,
        ...
        self.observation_space = gym.spaces.Dict({
            'image': gym.spaces.Box(low=0, high=255, shape=(224, 224, 3), dtype=np.float32),  # Use np.float32 here
            'vector': gym.spaces.Box(obs_range[0]*np.ones(23), obs_range[1]*np.ones(23), dtype=np.float32)  # Ensure consistency in dtype usage
        })
    ...
    def forward(self, image, **kwargs):
        ...
        #print(image.size)
        obs = {'image': image.reshape((224, 224, 3)), 'vector': obs}

...
class SharedBuffer(object):
    ...
    def __init__(self, array_len, array_type, np_array_type, buffer_len=DEFAULT_BUFFER_LEN, array_lock=True):
        ...
        self._array_lock = array_lock

        # Data is stored in a circular buffer of shared arrays
        self._data_buffer = []
        self._data_buffer_array = []

        if array_lock:
            for _ in range(self._buffer_len):
                self._data_buffer_array.append(Array(self._array_type, self.array_len))

            for i in range(self._buffer_len):
                self._data_buffer.append(np.frombuffer(self._data_buffer_array[i].get_obj(),
            ...
            for _ in range(self._buffer_len):
                self._data_buffer_array.append(RawArray(self._array_type, self.array_len))

            # use RawArray without internal lock if needed
            for i in range(self._buffer_len):
                self._data_buffer.append(np.frombuffer(self._data_buffer_array[i],
                                                       dtype=self.np_array_type))
        ...
        # Lock to ensure that changing tarray_lockhe `data_updated` as well as the data itself is atomic
        self._access_lock = Lock()
    
    def get_state(self):
        state = {}
        state['array_len'] = self.array_len 
        state['np_array_type'] = self.np_array_type 
        state['_buffer_len'] = self._buffer_len 
        state['_array_type'] = self._array_type 
        state['_array_lock'] = self._array_lock 
        state['_data_buffer_array'] = self._data_buffer_array
        state['_timestamp_buffer'] = self._timestamp_buffer
        state['_index_buffer'] = self._index_buffer
        state['_counter'] = self._counter
        state['_buffer_p'] = self._buffer_p
        state['_data_updated'] = self._data_updated
        state['_access_lock'] = self._access_lock

        return state
    
    def set_state(self, state):
        self.array_len = state['array_len'] 
        self.np_array_type = state['np_array_type']
        self._buffer_len  = state['_buffer_len']
        self._array_type  = state['_array_type']
        self._array_lock  = state['_array_lock']
        self._data_buffer_array = state['_data_buffer_array']
        self._timestamp_buffer = state['_timestamp_buffer']
        self._index_buffer = state['_index_buffer']
        self._counter = state['_counter']
        self._buffer_p = state['_buffer_p']
        self._data_updated = state['_data_updated']
        self._access_lock = state['_access_lock']

...
class FrankaPanda_Visual_Reacher_Dense(gym.Env):
    """
    Gym env for the real franka robot. Set up to perform the placement of a peg that starts in the robots hand into a slot
    """
    def __init__(self, dt=0.04, image_history_size=3, image_width=160, image_height=90, episode_length=8, camera_index=0, seed=9, size_tol=0.45):
        np.random.seed(seed)
        self.DT= dt
        self.dt = dt
        self.ep_time = 0
        ...
        self._image_history = np.zeros((self._image_height, self._image_width, 3 * image_history_size))
        ...
        shape=[self._image_height, self._image_width, 3 * image_history_size ],
        dtype=np.uint8)
    ...
    def reset(self):
        ...
        reset_pose['panda_joint4'] = np.random.uniform(-1.65, -1.6)
    ...
    def step(self, action, pose_vel_limit=0.3):
        ...
        if self._image_history_size > 1:
            self._image_history[:, :, 3:] = self._image_history[:, :, :-3]
            self._image_history[:, :, 0:3] = observation_robot["image"]
    ...
    #     return self._image_history.copy(), obs.copy(), reward, done, info

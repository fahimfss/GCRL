...
class BaseAgent:
    ...
    def _init_models(self, init_image_shape, init_proprioception_shape):
        self._rng, self._critic = init_critic(self._rng, 
                                              self._critic_lr, 
                                              init_image_shape, 
        ...
        self._rng, self._actor = init_actor(self._rng, 
                                            self._critic, 
                                            self._actor_lr, 
                                            init_image_shape, 
                                            init_proprioception_shape,  
    ...
    def get_actor_params(self): 
        if self._sync_mode:
            if self._resnet:
                variables = {'params': self._actor.params, 'batch_stats': self._actor.batch_stats}
            else:
                variables = {'params': self._actor.params}
            return variables
        else:
...
class AsyncSACRADAgent(BaseAgent):
    ...
    def __init__(self, args):
        ...
        self._actor_variables = self._actor_queue.get()
    ...
    def _init_async(self):
        ...
        if self._resnet:
            ...
            variables = {'params': self._actor.params}
        self._actor_queue.put(variables)
    ...
    def _async_tasks(self):
        while True:
            ...
            if self._update_step % self._actor_sync_freq == 0:
                if self._resnet:
                    variables = {'params': self._actor.params, 
                                 'batch_stats': self._actor.batch_stats}
                else:
                    variables = {'params': self._actor.params}
                self._actor_queue.put(variables)        
